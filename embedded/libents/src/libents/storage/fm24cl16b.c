 ...
 * Memory is organized as 8 pages × 256 bytes each, accessed via an 11-bit address:
 ...
 */
static fram_status write_chunk(fram_addr addr, const uint8_t* data, size_t len) {
  size_t written = 0;
...
static fram_status read_chunk(fram_addr addr, size_t len, uint8_t* data) {
  ...
  while (bytes_read < len) {
    ...
    size_t chunk = (len - bytes_read < bytes_in_page)
                   ? (len - bytes_read)
                   : bytes_in_page;
...
fram_status fm24cl16b_write(fram_addr addr, const uint8_t* data, size_t len) {
  ...
  while (bytes_written < len) {
    ...
    fram_status status = write_chunk(current_addr, data + bytes_written, chunk_len);
    if (status != FRAM_OK) {
      return status;
    }
...
fram_status fm24cl16b_read(fram_addr addr, size_t len, uint8_t* data) {
  ...
  while (bytes_read < len) {
    ...
    // Determine chunk size for this page
    size_t chunk_len = (len - bytes_read < bytes_in_page) 
                        ? (len - bytes_read) 
                        : bytes_in_page;
