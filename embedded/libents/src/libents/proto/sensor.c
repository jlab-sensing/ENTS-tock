...
SensorStatus EncodeRepeatedSensorMeasurements(Metadata meta,
  ...
  SensorStatus sensor_status =
      FormatRepeatedSensorMeasurements(meta, meas, count, &rep_meas);
  ...
  bool status =
      pb_encode(&ostream, RepeatedSensorMeasurements_fields, &rep_meas);
  if (!status) {
    return SENSOR_ERROR;
  }
...
SensorStatus RepeatedSensorMeasurementsSize(Metadata meta,
  ...
  // get size
  bool status =
      pb_get_encoded_size(size, RepeatedSensorMeasurements_fields, &rep_meas);
  if (!status) {
    return SENSOR_ERROR;
  }
...
SensorStatus DecodeRepeatedSensorMeasurements(
    const uint8_t* data, const size_t len,
    RepeatedSensorMeasurements* measurements) {
  pb_istream_t istream = pb_istream_from_buffer(data, len);
  bool status =
      pb_decode(&istream, RepeatedSensorMeasurements_fields, measurements);
  if (!status) {
    return SENSOR_ERROR;
  }
...
void PrintRepeatedSensorMeasurements(const RepeatedSensorMeasurements* msg) {
