...
**Both sides build clean. Nothing has run on hardware.**

Built on the jLab HARE server (Debian 13), which has the toolchains this was
originally written without.

esp32, PlatformIO 6.1.19, target esp32c3:

- `embedded/esp32/./build.sh` (`pio run`) succeeds across **all 12 environments**:
  `release`, `debug`, and the ten examples including `example_microsd`. Total
  4m55s, exit 0.
- `lib/module_handler/src/modules/microsd.cpp`, the file containing `WriteLog()`,
  compiles in every one of those 12 environments with **zero errors and zero
  warnings**. Object files were confirmed freshly built, not cached.
- `release` links and fits: RAM 4.3% (14004 of 327680 bytes), flash 19.8%
  (259690 of 1310720 bytes).
...
- **Real hardware. This is the only remaining gap.** No i2c transaction has ever
  been performed and no line has ever reached a physical SD card. That needs a
  board, `tockloader`, and a FAT32 card. Both halves compiling proves the code is
  well formed and links; it says nothing about whether the transport works, the
  esp32 answers at address 0x20, or the card mounts.
- The `__FILE__` prefix length under the real Tock build, see open item 2.
...
1. **Test on hardware.** The only remaining gap, and the only thing that closes
   the original jLab task. Flash the `.tab` with `tockloader`, run it with the
   esp32 attached and a FAT32 card inserted, and confirm a line actually appears
   in `/stm32.log`. Watch the esp32 serial at 115200 for
   `ModuleMicroSD::WriteLog` followed by `Wrote to and closed '/stm32.log'`,
   which separates an i2c failure from an SD failure.
